...
def inference(image):
	...
	tflite_model_dir = os.path.join(model_directory, 'model_converted.tflite')
	interpreter = tf.contrib.lite.Interpreter(model_path=tflite_model_dir)
	...
	max_pred_index = np.argmax(output[0])
	print("> Predicted value is", max_pred_index)
	print("> Probability:", output[0][max_pred_index])
...
def train(train_images_in, train_labels_in):
	...
	with open(filepath_log, 'w', encoding = 'utf-8') as f:
		...
		with tf.Session() as sess:
			...
			# Freeze the graph, converting weights into constants
			...
			# Remove unnecessary operations and optimize common operations
			...
			# Post-training quantization of the weights and conversion to TFLITE
			input_graph_def = tf.GraphDef()
			tflite_model_dir = os.path.join(model_directory, 'model_converted.tflite')	
