...
def weight_variable(shape, name):
    initial = tf.truncated_normal(shape, stddev=0.1, name = name) # Outputs random values from a truncated normal distribution
...
weights = {
	...
	"w_fconnected": weight_variable([7*7*64,100], 'w_fconnected'),
	"w_out":  weight_variable([100,10], 'w_out')
}
biases = {
	"b_conv1": bias_variable([32]),
	"b_conv2": bias_variable([64]),
	"b_fconnected": bias_variable([100]),
